  ...
  public void open(@NonNull final Result result) throws CameraAccessException {
    ...
    pictureImageReader =
        ImageReader.newInstance(
            resolutionFeature.getPreviewSize().getWidth(), resolutionFeature.getPreviewSize().getHeight(), ImageFormat.JPEG, 1);

    // Used to steam image byte data to dart side.
    imageStreamReader =
        ImageReader.newInstance(
            resolutionFeature.getPreviewSize().getWidth(), resolutionFeature.getPreviewSize().getHeight(), ImageFormat.YUV_420_888, 1);
